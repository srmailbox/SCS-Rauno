...
Cluster 2, 3, 1, 4.
The other four clusters are small groups of anomalous patterns.

```{r gr4HClustPlot}
gr4.hclust$hclust.cmplt = factor(gr4.hclust$hclust.cmplt, levels=c(2,3,1,4,5:8))
gr4.hclust %>% 
  pivot_longer(-hclust.cmplt, names_to="maze", values_to = "z") %>% 
#  mutate(hclust.cmplt = factor(hclust.cmplt, levels=c(5,2,3,1,4))) %>% 
  ggplot(aes(x=maze, y=z))+facet_wrap(vars(hclust.cmplt), ncol=4)+
  geom_boxplot()+
...
So it looks like LPA prefers the same basic modelling approach, but with either 2, 3, or 4 profiles.

All three models allow the classes to be of different sizes (in terms of the "range of scores that can be considered part of the class"), but will have equivalent covariance structures. All that varies between the models is the number of profiles.
...
These clearly seem to split along skill levels with group 1 (red - below average) < group 2 (blue - average to above average).
...
Curiously, this seems to break the "average to good" cluster into a larger one that is is more tightly clustered around the means, and a smaller one that has much wider standard deviations. The "wider" group does have somewhat higher average performances on some scales.
...
This adds a cluster of high performers.
...
```
Group 2 here are the high performers.
